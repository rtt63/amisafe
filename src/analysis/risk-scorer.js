...
function calculateRiskScore(prDiff, sizeAnalysis, claudeAnalysis, categorizedFiles) {
    let timeSpanHours = 0;
    ...
    const breakingChangesScore = claudeAnalysis.breakingChanges.length === 0
        ? 0
        : Math.min(10, Math.max(...claudeAnalysis.breakingChanges.map((bc) => bc.severity)) +
            Math.min(2, claudeAnalysis.breakingChanges.length * 0.5));
    const fatalErrorsScore = claudeAnalysis.fatalErrorRisks.length === 0
        ? 0
        : Math.min(10, Math.max(...claudeAnalysis.fatalErrorRisks.map((fe) => fe.severity)) +
            Math.min(2, claudeAnalysis.fatalErrorRisks.length * 0.5));
    const hasMigrations = categorizedFiles.critical.some((f) => /migration|\.sql$/i.test(f.path));
    const migrationCount = categorizedFiles.critical.filter((f) => /migration|\.sql$/i.test(f.path)).length;
    const migrationsScore = hasMigrations
        ? Math.min(10, 7.0 + (migrationCount > 1 ? Math.min(2, migrationCount * 0.5) : 0))
        : 0;
    const configFiles = categorizedFiles.critical.filter((f) => /config|\.env|docker|\.tf$|k8s|kubernetes/i.test(f.path));
    const configChangesScore = configFiles.length === 0
        ? 0
        : Math.min(10, 5.0 + Math.min(3, configFiles.length * 0.8));
    const timeSpanScore = timeSpanHours > 72
        ? 4.0
        : timeSpanHours > 48
            ? 3.0
            : timeSpanHours > 24
                ? 1.5
                : 0;
    const totalWeight = WEIGHTS.PR_SIZE +
        WEIGHTS.BREAKING_CHANGES +
        WEIGHTS.FATAL_ERRORS +
        WEIGHTS.MIGRATIONS +
        WEIGHTS.CONFIG_CHANGES +
        WEIGHTS.TIME_SPAN;
    const weightedSum = prSizeScore * WEIGHTS.PR_SIZE +
    ...
    if (timeSpanHours > 48) {
        recommendations.push("PR developed over multiple days - extra careful review needed");
...
function getTimeSpanHours(first, last) {
