...
const size_analyzer_1 = require("./size-analyzer");
async function analyzeWithClaude(files, depth, totalLines, apiKey) {
...
function buildPrompt(files, depth, totalLines) {
    const diffContent = formatDiff(files);
    switch (depth) {
        case size_analyzer_1.AnalysisDepth.SUPERFICIAL:
            return buildSuperficialPrompt(diffContent, totalLines);
        case size_analyzer_1.AnalysisDepth.SIMPLIFIED:
            return buildSimplifiedPrompt(diffContent, totalLines);
        case size_analyzer_1.AnalysisDepth.FULL:
            return buildFullPrompt(diffContent, totalLines);
        case size_analyzer_1.AnalysisDepth.DETAILED:
            return buildDetailedPrompt(diffContent, totalLines);
    }
}
function buildSuperficialPrompt(diff, lines) {
...
function buildSimplifiedPrompt(diff, lines) {
...
function buildFullPrompt(diff, lines) {
...
function buildDetailedPrompt(diff, lines) {
